...
import Logo from '../logo-fintek.png';
import { getWeatherByCity } from '../axios/WeatherAPI';
...
const Weather = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (!inputValue) {
      setError('Please enter a city name for getting the weather.');
      setWeatherData(null);
      ...
      try {
        ...
        const data = await getWeatherByCity(inputValue);
        if (data) {
          ...
          {isSubmitted && weatherData && (
            <div className="weather-display" aria-live="polite">
              <div className="weather-display weather-turquoise">
                ...
                <p className='date-time'>{`${weatherData.formattedDate} at ${weatherData.roundhour}`}</p>
                ...
                <div className="weather-details">
                  ...
                  <div className="weather-table">
                    ...
                    <div className="weather-table-body">
                      <div className="weather-table-cell">{weatherData.precip_mm}</div>
                      <div className="weather-table-cell">{weatherData.humidity}</div>
                      <div className="weather-table-cell">{weatherData.windchill_c}</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="temperature-table">
                  ...
                  <div className="latitude">Latitude {weatherData.latitude}</div>
                  <div className="longitude">Longitude {weatherData.longitude}</div>
                  <div className="accurate-to">accurate to {`${weatherData.formattedDate} at ${weatherData.hour}`}</div>
            </div>
          )}
        </div>
      </div>
